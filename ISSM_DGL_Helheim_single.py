...
def main():
    ...
    if args.local_rank == 0:
        ...
        print(f"## Total: {len(train_set)}; Train: {len(train_loader)*batch_size}; Val: {len(val_loader)*batch_size}; Test: {len(val_set)}")
    ...
    if args.initial == False:
        model_name = f"torch_dgl_Helheim_{args.model_type}_{n_nodes}_lr{lr}_in{in_channels}_ch{out_channels}_ft{hidden_channels}_gpu{1}"
    else:
        model_name = f"torch_dgl_HelheimINIT_{args.model_type}_{n_nodes}_lr{lr}_in{in_channels}_ch{out_channels}_ft{hidden_channels}_gpu{1}"
